...
class CustomReorderableListView extends StatefulWidget {
  ...
  /// (via [ScrollController.initialScrollOffset]), reading the current
  /// scroll position
  /// (via [ScrollController.offset]), or changing it
  /// (via [ScrollController.jumpTo] or
  ...
  /// underlying list.
  ///
  /// This [CustomReorderableListView] calls [onReorder] after a list child
  /// is dropped
  /// into a new position.
...
class _CustomReorderableListViewState extends State<CustomReorderableListView> {
  // We use an inner overlay so that the dragging list item doesn't draw
  // outside of the list itself.
...
class _ReorderableListContentState extends State<_ReorderableListContent>
  ...
  // Requests animation to the latest next index if it changes
  // during an animation.
  ...
  void _scrollTo(BuildContext context) {
    ...
    // If the context is off screen, then we request a scroll to make
    // it visible.
    if (!onScreen) {
      _scrolling = true;
  ...
  Widget _wrap(Widget toWrap, int index, BoxConstraints constraints) {
    ...
    // Places the value from startIndex one space before the element
    // at endIndex.
    ...
    Widget wrapWithSemantics() {
      ...
      // item gets dragged, the accessibility framework can preserve
      // the selected
      // state of the dragging item.
      //
      // We also apply the relevant custom accessibility actions for moving
      // the item
    ...
    // We wrap the drag target in a Builder so that we can scroll
    // to its specific context.
    return Builder(builder: (context) {
      return DragTarget<Key>(
        builder: buildDragTarget,
        onWillAccept: (toAccept) {
          setState(() {
            _nextIndex = index;
            _requestAnimationToNextIndex();
          });
          _scrollTo(context);
          // If the target is not the original starting point, then we
